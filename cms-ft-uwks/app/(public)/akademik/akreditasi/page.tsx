...
import { Award, FileText, Download, CheckCircle2, AlertCircle } from "lucide-react";

export const metadata: Metadata = {
  title: "Akreditasi Program Studi",
  description: "Status Akreditasi Program Studi Fakultas Teknik Universitas Wijaya Kusuma Surabaya",
};
...
export default async function AkreditasiPage() {
  const [prodiList, akreditasiList] = await Promise.all([
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="text-slate-700 text-sm md:text-base leading-relaxed space-y-5 text-justify">
        <p>
          Sebagai wujud komitmen terhadap mutu dan keunggulan pendidikan tinggi, seluruh program studi di lingkungan Fakultas Teknik Universitas Wijaya Kusuma Surabaya secara berkala diakreditasi oleh Badan Akreditasi Nasional Perguruan Tinggi (BAN-PT) maupun Lembaga Akreditasi Mandiri Program Studi Keteknikan (LAM Teknik).
        </p>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#002347] text-white text-xs uppercase tracking-wider font-semibold">
              ...
              <th className="py-3.5 px-4">No. SK Akreditasi</th>
                ...
                return (
                  <tr key={prodi.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-4 px-4 text-slate-600 text-xs whitespace-nowrap">
                      {akreditasi?.tanggalBerlaku
                        ? format(new Date(akreditasi.tanggalBerlaku), "d MMMM yyyy", {
                            locale: idLocale,
                          })
                        : "—"}
                    </td>
              ...
              >
                ...
                <div className="space-y-1.5 text-xs text-slate-600">
                  <div className="flex justify-between">
                    <span className="text-slate-400">No. SK:</span>
                ...
                {akreditasi?.fileSertifikat && (
                  <div className="pt-2">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full inline-flex items-center justify-center gap-2 px-4 py-2 text-xs font-bold text-[#002347] bg-[#E5B80B] hover:bg-[#d4a800] rounded-lg transition-colors shadow-xs"
                    >
                      <Download className="w-3.5 h-3.5" />
