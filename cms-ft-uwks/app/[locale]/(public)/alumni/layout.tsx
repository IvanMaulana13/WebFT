import type { Metadata } from "next";
import AlumniSidebar from "@/components/public/alumni-sidebar";
import AlumniHeader from "@/components/public/alumni-header";

export const metadata: Metadata = {
  title: {
    default: "Alumni | Fakultas Teknik UWKS",
    template: "%s | Fakultas Teknik UWKS",
  },
  description: "Ikatan Alumni, Tracer Study, dan Pengembangan Karir Alumni Fakultas Teknik UWKS",
};

export default function AlumniLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  return (
    <main className="w-full">
      <AlumniHeader />
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 md:py-16 grid grid-cols-12 gap-6 md:gap-8">
        <AlumniSidebar />
        <article className="col-span-12 md:col-span-8 lg:col-span-9 bg-white rounded-xl p-6 sm:p-8 lg:p-12 shadow-sm border border-slate-200">
          {children}
        </article>
      </div>
    </main>
  );
}
