...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("profil.pimpinan");
  return {
...
function formatPeriod(mulai?: string | Date | null, selesai?: string | Date | null, isEn = false) {
  if (!mulai && !selesai) return "";
  const yearMulai = mulai ? new Date(mulai).getFullYear() : "";
  const yearSelesai = selesai ? new Date(selesai).getFullYear() : (isEn ? "Present" : "Sekarang");
  return `${isEn ? "Period" : "Periode"} ${yearMulai} - ${yearSelesai}`;
}

export default async function PimpinanFakultasPage() {
  const [pimpinanList, tProfil, tCommon] = await Promise.all([
    fetchPublicData<PimpinanItem[]>("/api/pimpinan").then((res) => res || []),
      ...
      <section className="relative h-64 flex items-center overflow-hidden bg-[#002347] text-white">
        ...
        <div className="relative z-20 max-w-6xl mx-auto px-6 w-full">
          <nav className="flex items-center gap-2 mb-3 text-xs uppercase tracking-wider text-white/80">
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-[#E5B80B] font-bold">{tProfil("pimpinan.breadcrumb")}</span>
          </nav>
          <h1 className="text-3xl sm:text-4xl font-bold font-sans uppercase">
            {tProfil("pimpinan.title")}
          </h1>
        </div>
      </section>
