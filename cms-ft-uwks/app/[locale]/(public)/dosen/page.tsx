...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("profil.dosen");
  return {
...
export default async function DosenPage() {
  const [dosenList, tProfil, tCommon] = await Promise.all([
    fetchPublicData<DosenItem[]>("/api/dosen").then((res) => res || []),
      ...
      <section className="relative h-64 flex items-center overflow-hidden bg-[#002347] text-white">
        ...
        <div className="relative z-20 max-w-6xl mx-auto px-6 w-full">
          <nav className="flex items-center gap-2 mb-3 text-xs uppercase tracking-wider text-white/80">
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-[#E5B80B] font-bold">{tProfil("dosen.breadcrumb")}</span>
          </nav>
