...
import { FileText, Upload, Trash2, Calendar, Loader2, AlertCircle } from "lucide-react";
...
// Main Component
// ─────────────────────────────────────────────
export function KalenderAkademikPanel() {
  ...
  // ─── Query ─────────────────────────────────
  const { data, isLoading } = useQuery<{ data: KalenderRecord | null }>({
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch("/api/akademik/kalender", {
        ...
        body: JSON.stringify({ fileUrl, tahunAjaran }),
      });
      if (!res.ok) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch("/api/akademik/kalender", { method: "DELETE" });
      if (!res.ok) {
        const err = await res.json();
  ...
  const openDialog = () => {
    ...
    setTahunAjaran(record?.tahunAjaran ?? "");
    setDialogOpen(true);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        {record?.fileUrl ? (
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-500 mt-0.5">
                  Tahun Ajaran: <span className="font-medium text-gray-700">{record.tahunAjaran ?? "—"}</span>
                </p>
          ...
          <div className="text-center py-10">
            ...
            <p className="font-medium text-gray-700">Belum ada kalender akademik</p>
            <p className="text-sm text-gray-400 mt-1 mb-4">
              Upload file PDF kalender akademik untuk ditampilkan.
            </p>
      ...
      <div className="flex items-start gap-3 p-4 rounded-lg bg-amber-50 border border-amber-200 text-sm text-amber-800">
        ...
        <p>
          Hanya satu file kalender akademik yang aktif. Mengunggah file baru akan
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {record?.fileUrl ? "Ganti File Kalender Akademik" : "Upload Kalender Akademik"}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              <Label>File Kalender (PDF)</Label>
              <DocumentUpload
                value={fileUrl}
                onChange={(url) => setFileUrl(url)}
                onClear={() => setFileUrl("")}
                label="Klik untuk upload PDF kalender"
              />
            </div>
          </div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!fileUrl || !tahunAjaran || saveMutation.isPending}
            >
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              File kalender akademik akan dihapus. Tindakan ini tidak dapat dibatalkan.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
