...
import { GraduationCap, Calendar, ChevronRight, ChevronLeft } from "lucide-react";
import type { Berita } from "@/lib/db/schema";

interface BeasiswaClientProps {
  initialData: Berita[];
}

const ITEMS_PER_PAGE = 8;

export default function BeasiswaClient({ initialData }: BeasiswaClientProps) {
  ...
  // Helper to extract text snippet from html or markdown
    ...
    return (
      <div className="bg-[#F8F9FA] rounded-xl p-12 text-center border border-slate-200">
        ...
        <p className="text-slate-500 font-medium">
          Belum ada informasi beasiswa yang dipublikasikan.
        </p>
      </div>
    );
  }
          ...
          >
            ...
            <div className="relative w-full h-48 bg-slate-100 overflow-hidden">
              {item.thumbnailUrl ? (
                ...
                <div className="w-full h-full bg-gradient-to-br from-blue-50 via-slate-50 to-indigo-50 flex items-center justify-center text-[#002C5F]">
                  <GraduationCap className="w-12 h-12 text-[#E5B80B]" />
                </div>
              )}
