...
export default function LombaClient({ initialData }: LombaClientProps) {
  ...
  const renderLombaCard = (item: Lomba, isOpen: boolean) => {
    return (
      ...
      >
        {/* Poster / Thumbnail */}
        <div className="w-full md:w-44 h-40 md:h-auto rounded-lg bg-slate-100 overflow-hidden shrink-0 relative flex items-center justify-center border border-slate-200">
          {item.posterUrl ? (
            <Image
              src={item.posterUrl}
              alt={item.namaLomba}
            ...
            <div className="flex flex-col items-center justify-center text-slate-300 p-4 text-center">
              <Medal className="w-10 h-10 text-[#E5B80B] mb-1" />
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-600 mb-3 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
            ...
            <span>
              <strong className="text-[#002347]">Periode Pendaftaran:</strong>{" "}
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="closed">Sudah Berakhir</option>
            </select>
          </div>
        </div>

        <div className="text-xs font-medium text-slate-500 self-end sm:self-center">
          Total: <span className="font-bold text-[#002347]">{filteredData.length}</span> lomba
        </div>
      </div>
      ...
      {filteredData.length === 0 && (
        <div className="bg-[#F8F9FA] rounded-xl p-12 text-center border border-slate-200">
          ...
          <p className="text-slate-600 font-semibold mb-1">
            Belum ada informasi lomba untuk filter yang dipilih.
          </p>
          <p className="text-xs text-slate-400">
            Silakan ubah pilihan filter di atas untuk melihat agenda lomba lainnya.
          </p>
        </div>
      )}
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2 pb-2 border-b border-emerald-200">
            ...
            <h3 className="text-lg font-bold text-[#002347] uppercase tracking-tight flex items-center gap-2">
              <span>Pendaftaran Masih Dibuka</span>
        ...
        <section className="space-y-4 pt-4">
          <div className="flex items-center gap-2 pb-2 border-b border-slate-200">
            ...
            <h3 className="text-lg font-bold text-slate-600 uppercase tracking-tight flex items-center gap-2">
              <span>Pendaftaran Sudah Berakhir</span>
